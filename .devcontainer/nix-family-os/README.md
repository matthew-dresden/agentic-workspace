...
This directory is a **common catalog asset** — it is automatically copied into every project's `.devcontainer/nix-family-os/` when a devcontainer is set up via `workspace setup-devcontainer`. It contains scripts for managing tinyproxy as a background daemon on your macOS or Linux host, required for devcontainer proxy support with an upstream corporate proxy.
...
3. A runtime config is written to `~/.workspace-proxy/tinyproxy.conf`
...
All logs are stored in `~/.workspace-proxy/`:

| File | Purpose | Location |
|------|---------|----------|
| **Output Log** | Standard tinyproxy output | `~/.workspace-proxy/tinyproxy.log` |
| **PID File** | Process ID of running daemon | `~/.workspace-proxy/tinyproxy.pid` |
| **Runtime Config** | Generated config from template | `~/.workspace-proxy/tinyproxy.conf` |

### View logs in real-time

**Watch output log:**
```bash
tail -f ~/.workspace-proxy/tinyproxy.log
```

**View last 50 lines:**
```bash
tail -50 ~/.workspace-proxy/tinyproxy.log
```
...
```bash
# Clear all logs
rm ~/.workspace-proxy/*.log

# Clear just output log
rm ~/.workspace-proxy/tinyproxy.log
```
...
**Check error logs:**
```bash
cat ~/.workspace-proxy/tinyproxy.log
```
...
**Check system logs:**
```bash
tail -100 ~/.workspace-proxy/tinyproxy.log
```
...
<dict>
    ...
    <string>/Users/YOUR_USERNAME/.workspace-proxy/launchd-stdout.log</string>
    <key>StandardErrorPath</key>
    <string>/Users/YOUR_USERNAME/.workspace-proxy/launchd-stderr.log</string>
</dict>
</plist>
```
...
4. **"Failed to start proxy"**
   - Check logs: `cat ~/.workspace-proxy/tinyproxy.log`
...
- The generated runtime config is stored in `~/.workspace-proxy/` (not in the repo)
