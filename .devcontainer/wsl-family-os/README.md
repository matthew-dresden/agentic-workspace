...
This directory is a **common catalog asset** — it is automatically copied into every project's `.devcontainer/wsl-family-os/` when a devcontainer is set up via `workspace setup-devcontainer`. It contains scripts for managing tinyproxy as a background daemon on your Windows/WSL host, required for devcontainer proxy support with an upstream corporate proxy.
