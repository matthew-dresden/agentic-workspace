"""Version checking and update utilities for the Agentic Workspace CLI."""
...
from workspace_cli.utils.ui import COLORS
...
def _debug_log(message):
    ...
    if os.getenv("WORKSPACE_DEBUG_UPDATE") == "1":
...
def _get_latest_version():
    ...
    try:
        req = Request("https://pypi.org/pypi/agentic-workspace/json")
        req.add_header("User-Agent", f"agentic-workspace/{__version__}")
...
def _is_installed_with_pipx():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if "agentic-workspace" in data.get("venvs", {}):
                return True
    except (
...
def _show_manual_upgrade_instructions(install_type):
    ...
    if install_type == "pipx":
        print("\nUpgrade with pipx:")
        print("  pipx upgrade agentic-workspace")

    elif install_type == "pipx editable":
        print("\nUpgrade editable installation:")
        print("  cd /path/to/workspace-cli")
        print("  git pull")
        print("  pipx reinstall -e .")
        print()
        print("Or switch to regular pipx installation:")
        print("  pipx uninstall agentic-workspace")
        print("  pipx install agentic-workspace")
        ...
        print("  pip uninstall agentic-workspace")
        print("  pipx install agentic-workspace")
...
def check_for_updates():
    ...
    if os.getenv("WORKSPACE_SKIP_UPDATE") == "1":
